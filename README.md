...
| 14 | 翻訳モデルの実践（Helsinki-NLP） | ✅ 完了 |
| 15 | テキスト生成の実践・生成パラメータ調整 | ✅ 完了 |
| 16 | ミニプロジェクト：翻訳→要約パイプライン | ✅ 完了 |
| 17 | 振り返り・次のステップの展望 | ✅ 完了 |
...
翻訳はエンコーダ＋デコーダ両方が必要な構造であることを確認。Helsinki-NLPのMarianMTモデルで日本語→英語翻訳を実装。transformers v5ではpipeline("translation")が使えないため直接ロードする方法を採用。BLEUスコア（1〜4gramの幾何平均）で翻訳精度を評価。

**キーワード**：MarianMT・AutoModelForSeq2SeqLM・BLEUスコア・n-gram・sacrebleu・内部表現・Cross-Attention

---

### Day 15：翻訳→要約パイプライン
Helsinki-NLPとBARTを組み合わせた翻訳→要約のエンドツーエンドパイプラインを実装。max_position_embeddings=512の制限で長文が途中で切れる問題を発見し、句点で文章を分割するchunked翻訳で解決。専門用語の翻訳精度の限界も確認。

**キーワード**：エンドツーエンドパイプライン・chunked翻訳・max_position_embeddings・attention_mask・BART

---

### Day 16・17：振り返り・次のステップ
17日間の学習を振り返り、次のステップを整理。MLエンジニアリングトラックとして、Kaggleコンペ参加・HuggingFaceでのファインチューニング・個人プロジェクトのGitHub公開を直近3ヶ月の目標として設定。
